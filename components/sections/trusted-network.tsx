...
export function TrustedNetwork() {
    return (
        <section className="overflow-hidden bg-gradient-to-b from-secondary/50 to-background px-6 py-16 text-center">
            ...
            <div className="mx-auto mt-10 flex max-w-5xl flex-wrap items-start justify-center gap-8">
                {partners.map(([file, name]) => (
                    <div key={name} className="flex w-32 flex-col items-center gap-2">
                        <div className="flex h-12 w-full items-center justify-center rounded-xl border bg-card p-3"><Image src={`/tenderiq/${file}`} alt={name} width={108} height={32} className="max-h-8 w-auto object-contain grayscale opacity-50" /></div>
