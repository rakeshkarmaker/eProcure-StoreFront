...
export function TenderResultCard({ tender }: { tender: Tender }) {
    return (
        <Card className="transition duration-200 hover:ring-primary/40 hover:shadow-lg">
            <CardHeader>
                ...
                <CardAction><Button size="icon" asChild><Link href={`/tender/${tender.id}`} aria-label={`View ${tender.title}`}><ArrowRight /></Link></Button></CardAction>
            </CardHeader>
            <CardContent className="grid gap-2 text-xs text-muted-foreground sm:grid-cols-2 lg:grid-cols-4">
                ...
                <span className="flex items-center gap-1.5 font-semibold text-foreground"><Clock3 className="size-3.5" />{tender.days} days left</span>
            </CardContent>
            <CardFooter className="justify-between text-xs text-muted-foreground"><span>Estimated budget</span><span>Ends in {tender.days} days</span></CardFooter>
        </Card>
    )
}
